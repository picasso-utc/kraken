...
from core.permissions import IsAuthenticatedUser, IsMemberUser
...
def get_history_surveys(request):
    """
    Renvoie les sondages archivés, c'est-à-dire
...
def delete_survey(request, pk=None):
    """
        ...
        informations confidentielles que sont le vote d'une personne pour un item
...
def vote_survey(request, survey_id=None, item_id=None):
    ...
    Dans le cas d'un sondage multichoix on s'assure que l'utilisateur n'a pas
    ...
    if len(surveys) > 0:
        ...
        # Si multichoix autorisé on vérifie simplement que l'utilisateur n'a pas déjà voté pour cet item
