...
class PayutcClient:
    ...
    def login_admin(self):
        ...
        if not sessionid:
            sessionid = self.get_config('sessionid')['sessionid']
        if not sessionid:
            ...
            cache.set('sessionid', sessionid, 120 * 60)
    ...
    def patch_api_rest(self, service, method, id, params=None, **data):
        ...
        Si par exemple un article a un nom et qu'on n'envoie pas de nom
        ...
        Ici la méthode est générique tu peux envoyer n'importe quel service, méthode,
        paramètre.
        Elle est notamment utilisée dans Perm/ Models.py pour les articles
        Jete un coup d'œil si ça te paraît utile pour faire une mise à jour pour
        autre chose
        """
